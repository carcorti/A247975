% ============================================================
%  Submission draft (target: Mathematics of Computation, AMS)
...
\begin{document}
	...
	\begin{abstract}
		...
		We determine $a(m)$ for $99\,987$ of the values $m = 1, 2, \ldots,
		100\,000$, with final search bound $n \leq 1.6 \times 10^{10}$;
		the remaining $13$ values are unresolved within this bound.
		The initial computation (search bound $n \leq 2 \times 10^9$)
		resolved $99\,951$ cases with $49$ unresolved; a targeted
		post-submission extension resolved $36$ of those $49$ cases,
		reducing the unresolved count to~$13$.
		Among the $99\,987$ resolved values we identify $434$ exceptional
		resolved cases where $a(m) > 10^8$, and document the outstanding
		Cram\'er-type ratio $a(m)/(m \log m)$
		(all logarithms are natural) at $m = 11\,924$ with
		$a(m) = 6\,119\,832\,581$ (ratio $\approx 54\,679$, the global
		maximum), eclipsing the previous record holder $m = 4703$
		(ratio $\approx 19\,111$).
		The largest absolute value of $a(m)$ is now
		$a(55\,468) = 12\,790\,824\,118$.
		...
		The $13$ unresolved values include an arithmetic cluster
		$\{37\,249, 37\,253, 37\,949\}$ and two cases ($m = 22\,802$,
		$m = 23\,995$) with estimated Cram\'er ratios exceeding $65\,000$.
		Divisibility was independently verified for all $434$ exceptional
		cases via \texttt{SymPy} ($398$ from the initial computation and
		all $36$ from the extended search); minimality was independently
		verified for $290$ cases
		...
		$a(m) > 10^9$ from the initial computation.
% REVISED [OBS-002]: Removed AI-assistance sentence from abstract (remains in Acknowledgments)
	\end{abstract}
...
% REVISED [OBS-009]: Added statement that no competing computations exist
	...
	distribution of $a(m)$.
	Third, we identify and characterize $13$ values of $m$ for which the
	conjecture resists verification within our final search bound
	$n \leq 1.6 \times 10^{10}$, all sharing a distinctive modular
	obstruction pattern; among these, three values form an arithmetic
	cluster suggesting a common structural obstruction.
...
% REVISED [OBS-002]: Added forward reference to obstruction rate definition
	...
	reaches $n \leq 1.6 \times 10^{10}$, so
	$s = m + n$ can reach $\approx 1.6 \times 10^{10}$ and the
	product $p_n^2 \bmod s$ requires intermediate values that may exceed
	$2^{63}$.
	Accordingly, the extended-search programs use \texttt{\_\_int128}
	for the modular squaring step $p_n^2 \bmod s$, while all other
	arithmetic remains in \texttt{int64\_t}.
	...
	After the initial computation (Phase~6) left $49$ values unresolved
	at bound $n \leq 2 \times 10^9$, a targeted post-submission search
	progressively extended the bound to $n \leq 1.6 \times 10^{10}$,
	resolving $36$ of the $49$ cases and reducing the unresolved count
	to~$13$.

	The extended search used a family of programs
	(\texttt{sun41\_v12.c}--\texttt{sun41\_v12h.c}) with a fundamentally
	different architecture from the Phase-6 program: instead of building
	a global prime table (which would require $\sim 20$\,GB of RAM at
	bound $10^{10}$), these programs use a segmented sieve with no
	persistent prime table, computing primes on the fly.
	The starting prime $p_{\mathrm{START\_N}}$ is computed once; all
	subsequent primes are enumerated within segments of
	$2 \times 10^7$ odd slots ($2.50$\,MB per thread).
	Peak RAM stays below $100$\,MB regardless of the search bound.
	Parallelism is achieved via OpenMP on $16$ threads.

	The modular squaring step uses \texttt{\_\_int128} for
	$p_n^2 \bmod s$ when $n > \sim 3.5 \times 10^9$ (i.e., when
	$p_n^2$ may exceed $2^{63}-1$); all other arithmetic remains in
	\texttt{int64\_t}.

	Table~\ref{tab:extended} summarises the extended search.
	The total additional computation required approximately $129$~minutes
	on an AMD Ryzen~9 7940HS (UM790 Pro, $16$ OpenMP threads).
	The search was halted after Ext-8 ($n \leq 1.6 \times 10^{10}$),
	which resolved $0$ additional cases, indicating a natural stopping
	point: the remaining $13$ cases have
	estimated Cram\'er ratios $C(m) > 14\,000$ and would likely require
	$n \gg 10^{10}$.

% REVISED [OBS-001]: Renumbered extended-search runs to Ext-1..Ext-8 to avoid collision with initial Phases 1-6
	\begin{table}[ht]
		\centering
		\caption{Extended search: progressive resolution of the $49$ initially
			unresolved cases.  ``+Resolved'' denotes newly resolved cases in
			each run.  Run labels ``Ext-$k$'' distinguish the extended-search
			runs from the initial computation phases (Phases~1--6).}
		\label{tab:extended}
		\small
		\begin{tabular}{llrrrl}
			\toprule
			Run & Bound $n$ & +Resolved & Still open & Time (min) & Code \\
			\midrule
			Phase~6 (initial) & $\leq 2 \times 10^9$  & ---& 49 & 10 & \texttt{sun41\_v6b.c} \\
			Ext-1 & $\leq 3 \times 10^9$  & 11 & 38 & 18 & \texttt{sun41\_v12.c} \\
			Ext-2 & $\leq 4 \times 10^9$  &  4 & 34 & 16 & \texttt{sun41\_v12b.c} \\
			Ext-3 & $\leq 5 \times 10^9$  &  4 & 30 & 13 & \texttt{sun41\_v12c.c} \\
			Ext-4 & $\leq 6 \times 10^9$  &  5 & 25 & 11 & \texttt{sun41\_v12d.c} \\
			Ext-5 & $\leq 7 \times 10^9$  &  4 & 21 & 11 & \texttt{sun41\_v12e.c} \\
			Ext-6 & $\leq 10 \times 10^9$ &  6 & 15 & 27 & \texttt{sun41\_v12f.c} \\
			Ext-7 & $\leq 13 \times 10^9$ &  2 & 13 & 19 & \texttt{sun41\_v12g.c} \\
			Ext-8 & $\leq 16 \times 10^9$ &  0 & 13 & 13 & \texttt{sun41\_v12h.c} \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}
		...
		$434$ exceptional resolved cases ($a(m) > 10^8$), the values $p_m$
		...
		All $434$ verified without error: $398$ cases from the initial
		computation and all $36$ cases from the extended search.
		For the $36$ extended-search cases, $p_{a(m)}$ values range from
		$\approx 5.1 \times 10^{10}$ to $\approx 3.3 \times 10^{11}$;
		...
		C~implementation.
% REVISED [OBS-004]: Added explicit minimality caveat for 36 extended-search cases
		In particular, for the $36$ extended-search cases, minimality rests
		solely on the C~search implementation; independent minimality
		verification (Method~3) was performed only for cases from the
		initial computation.
% REVISED [OBS-003]: Strengthened minimality caveat with future-work commitment
		An independent minimality check for these $36$ cases (e.g., by
		extending \texttt{verify\_min.c} to cover the extended-search range)
	...
	In total, $10\,724$ individual verifications were performed across all
	...
	For $m = 1, \ldots, 100\,000$, we determined $a(m)$ in $99\,987$ cases:
	$99\,951$ in the initial computation (Phase~6, bound
	$n \leq 2 \times 10^9$) and $36$ additional cases in the extended
	search (bound $n \leq 1.6 \times 10^{10}$; see
	Section~\ref{sec:extended}).
	The remaining $13$ values are listed and discussed in
	...
	($\approx 65\,000$, computed from the $99\,987$ resolved values).
	There are $E_{\mathrm{res}}(10^5) = 434$ such resolved exceptional
	cases ($398$ from the initial computation plus all $36$ cases resolved
	in the extended search, each of which has $a(m) > 2 \times 10^9$).
	Since the $13$ unresolved values all satisfy
	$a(m) > 1.6 \times 10^{10}$ by definition of the final search bound,
	the true exceptional count satisfies $E(10^5) \geq 447$, where
	\[
	E(M) \;=\; E_{\mathrm{res}}(M)
	...
	The complete dataset (all $99\,987$ resolved values and the list of
	$13$ unresolved~$m$) has been submitted to the OEIS as an extension of
	...
	\begin{table}[ht]
		...
		\begin{tabular}{rrrr}
			...
			\midrule
			55\,468 & 12\,790\,824\,118 &  685\,453 & 21\,110 \\
			51\,816 & 11\,824\,762\,849 &  636\,277 & 21\,022 \\
			67\,447 &  8\,688\,185\,334 &  847\,621 & 11\,585 \\
			98\,258 &  8\,564\,068\,296 & 1\,275\,011 &  7582 \\
			97\,251 &  7\,850\,344\,255 & 1\,260\,901 &  7029 \\
			67\,203 &  7\,747\,012\,646 &  844\,253 & 10\,371 \\
			55\,874 &  7\,689\,816\,992 &  690\,953 & 12\,591 \\
			82\,096 &  7\,393\,138\,089 & 1\,049\,519 &  7958 \\
			76\,761 &  6\,814\,926\,932 &  975\,497 &  7893 \\
			86\,557 &  6\,670\,175\,973 & 1\,111\,393 &  6778 \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	$m = 11\,924$ now holds the global maximum
	($C \approx 54\,679$), nearly three times the previous record holder
	$m = 4703$ ($C \approx 19\,111$, now ranked~6th).
	...
	\begin{table}[ht]
		...
		\begin{tabular}{rrrrr}
			...
			\midrule
			1 & 11\,924 & 6\,119\,832\,581 & 127\,289 & 54\,679 \\
			2 & 22\,186 & 6\,418\,823\,416 & 251\,809 & 28\,911 \\
			3 & 19\,234 & 5\,206\,723\,528 & 215\,317 & 27\,442 \\
			4 & 55\,468 & 12\,790\,824\,118 & 685\,453 & 21\,110 \\
			5 & 51\,816 & 11\,824\,762\,849 & 636\,277 & 21\,022 \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	For $m = 11\,924$: $p_m = 127\,289$,
	$a(m) = 6\,119\,832\,581$,
	$s = m + a(m) = 6\,119\,844\,505$.
	For $m = 4703$ (previous record, now rank~6): $p_m = 45\,329$,
	...
	\right)^{-1}.
	\]
	The Hill statistic, computed over the top $10\%$ of the $99\,951$
	values from the initial computation
	($k = 9995$, threshold $a(m) > 1\,142\,143$), takes the value
	\[
	\hat{\alpha}_{\mathrm{Hill}} = 0.713.
	\]
% REVISED [OBS-013]: Strengthened Hill estimator dataset stability note
	The $36$ values from the extended search are all in the extreme tail
	($a(m) > 2 \times 10^9$) and represent $0.04\%$ of the dataset;
	their inclusion does not materially change the Hill estimate
	(the statistic is driven by the top $10\%$ of the initial dataset,
	and $36$ additional points in the far tail shift $\hat{\alpha}$ by
	less than $0.002$).
	...
	(computed from the $99\,987$ resolved values) is
	\[
	\widetilde{C} \approx 0.157,
	\]
	stable and well-defined across the full range; the $36$ newly resolved
	...
	but the growth factor of $217$ from $E_{\mathrm{res}}(10^4) = 2$ to
	$E_{\mathrm{res}}(10^5) = 434$ is far larger than any simple log-power
	...
	The outstanding Cram\'er-type ratio of $m = 11\,924$
	(ratio $\approx 54\,679$, the global maximum over
	$m \leq 100\,000$) points toward richer arithmetic structure.
	The previous record holder $m = 4703$ (ratio $\approx 19\,111$)
% REVISED [OBS-006]: Softened speculative "systematic suppression" claim
	was surpassed by a factor of~$2.9$; specific properties of
	$p_{11\,924} = 127\,289$ and its
	...
	The $49$ values of $m \leq 100\,000$ that were unresolved in the
	initial computation ($n \leq 2 \times 10^9$) were subjected to the
	extended search described in Section~\ref{sec:extended}.
	Of these, $36$ were resolved, leaving $13$ values unresolved
	within the final search bound $n \leq 1.6 \times 10^{10}$.
...
% REVISED [OBS-005]: Added explicit definition of obstruction rate
...
\begin{table}[ht]
	\centering
	\caption{The $13$ unresolved values of $m \leq 100\,000$, with $p_m$,
		...
		No solution was found with $n \leq 1.6 \times 10^{10}$.}
	\label{tab:notfound}
	\small
	\begin{tabular}{rrrr}
		\toprule
		$m$ & $p_m$ & Obstr.\ (\%) & $C(m) >$ \\
		\midrule
		22\,802 &  259\,577 & 75.7 & 69\,927 \\
		23\,995 &  274\,453 & 75.8 & 66\,115 \\
		37\,249 &  443\,837 & 76.2 & 40\,810 \\
		37\,253 &  443\,873 & 76.2 & 40\,805 \\
		37\,949 &  453\,181 & 76.3 & 39\,987 \\
		43\,297 &  523\,007 & 76.4 & 34\,615 \\
		48\,834 &  596\,419 & 76.5 & 30\,348 \\
		53\,963 &  665\,179 & 76.7 & 27\,212 \\
		66\,257 &  831\,301 & 76.9 & 21\,753 \\
		69\,546 &  876\,191 & 76.9 & 20\,634 \\
		76\,868 &  977\,087 & 77.0 & 18\,502 \\
		82\,300 & 1\,052\,459 & 77.0 & 17\,177 \\
		98\,379 & 1\,276\,747 & 77.2 & 14\,146 \\
		\bottomrule
	\end{tabular}
\end{table}

All $13$ cases exhibit obstruction rates in the range $75.7$--$77.2\%$,
consistent with the $75.1$--$77.3\%$ range observed for the original
$49$ unresolved cases.
The extended search resolved $36$ of those $49$ cases
...
The $13$ remaining cases are precisely those with the highest estimated
Cram\'er ratios among the original~$49$.

\paragraph{Arithmetic cluster.}
Three of the $13$ cases form an arithmetic cluster:
$m \in \{37\,249,\; 37\,253,\; 37\,949\}$.
The first two differ by only~$4$; the third lies $700$ above
$m = 37\,249$.
Their primes are $p_{37\,249} = 443\,837$,
$p_{37\,253} = 443\,873$, and $p_{37\,949} = 453\,181$.
The proximity of these values suggests a common arithmetic obstruction
--- possibly a shared pattern in the residue classes of $p_m \pmod{s}$
--- rather than independent coincidence.
% REVISED [OBS-018]: Added remark on modular pattern investigation for unresolved cases
A systematic investigation of the modular obstruction patterns shared by
these $13$ cases (in particular, their quadratic non-residue structure)
may yield further insight and is left for future work.

\paragraph{Extreme cases.}
The two most resistant cases, $m = 22\,802$ and $m = 23\,995$,
have estimated Cram\'er ratios exceeding $65\,000$, surpassing the
resolved record $C(11\,924) \approx 54\,679$.
If their true $a(m)$ values scale proportionally to the Cram\'er ratio
(as suggested by the pattern among resolved cases), they may require
$n \sim 2 \times 10^{10}$ or beyond.

\paragraph{Resolution curve.}
The extended search resolved cases at a decreasing rate:
$11$ cases in the first $10^9$ units of search range, but only $2$ in
the range $n = 1.0 \times 10^{10}$ to $1.3 \times 10^{10}$, and $0$
in the range $n = 1.3 \times 10^{10}$ to $1.6 \times 10^{10}$.
This rapid exhaustion of resolvable cases suggests that the $13$
remaining values are genuinely harder and would require a substantially
larger bound.

We cannot rule out that some of these $13$ cases represent genuine
...
$C(11\,924) \approx 54\,679$ at $n \approx 6.1 \times 10^9$, and the
pattern of progressive resolution suggests that the $13$ remaining
...
We have determined $a(m)$ for $99\,987$ values of $m \leq 100\,000$,
with $13$ values remaining unresolved within our final search bound
$n \leq 1.6 \times 10^{10}$.
The initial computation ($n \leq 2 \times 10^9$) resolved $99\,951$
cases; a targeted post-submission extension resolved $36$ of the
$49$ initially unresolved cases, reducing the count to~$13$.
...
density of exceptional cases, and a new outstanding case at
$m = 11\,924$ whose Cram\'er-type ratio ($\approx 54\,679$) is
the global maximum over the entire range studied, nearly three times
the previous record holder $m = 4703$ ($\approx 19\,111$).

The $13$ unresolved cases include an arithmetic cluster
$\{37\,249,\; 37\,253,\; 37\,949\}$ and two extreme cases
($m = 22\,802$ and $m = 23\,995$) with estimated Cram\'er ratios
exceeding $65\,000$.
The progressive resolution of the original $49$ unresolved cases
--- from $49$ at bound $2 \times 10^9$ to $13$ at bound
$1.6 \times 10^{10}$ --- demonstrates that the conjecture holds for
...
characterization of the high-obstruction cases (particularly the
cluster structure), an extended
computational search for the $13$ remaining unresolved values, and a
refined probabilistic model for the distribution of $a(m)$
(see also \cite{Granville1995} for a discussion of corrections to
the classical Cram\'er heuristic).

% REVISED [OBS-003]: Added footnote about pending Zenodo/OEIS status
% REVISED [OBS-026]: Added CSV datasets and verification logs to data listing
\paragraph{Data availability.}
The complete dataset ($99\,987$ resolved values and $13$ unresolved
$m$-values) has been submitted to the OEIS as an extension of the
b-file for A247975.%
\footnote{The Zenodo archive and the OEIS b-file extension were
pending publication at the time of submission; Zenodo metadata will be
updated to reflect the final submission target.}
The C~source code for the main search (\texttt{sun41\_v6b.c}), the
extended-search programs
(\texttt{sun41\_v12.c}--\texttt{sun41\_v12h.c}), the
independent verification sieve (\texttt{verify\_min.c}), the
Python/SymPy verification scripts, the complete CSV datasets,
and the full machine-readable verification logs are publicly
available at:
\begin{itemize}
...
\end{itemize}
% REVISED [OBS-006]: Added repository synchronization note
...
% REVISED [OBS-015]: Added access date to OEIS reference
...
% REVISED [OBS-007]: Added separate Tenenbaum bibliography entry
