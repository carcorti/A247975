/*
 * sun41_v6b.c  --  Computational verification of Sun's Conjecture 4.1(i)
 *
 ...
 *       src/sun41_v6b.c -o sun41_v6b -lm
 *
 * Usage:
 ...
 *
 * CHANGES v6b (domain extension — large bound support):
 *   v6 declared g_prime_count as `int` (32-bit), silently overflowing for
 *   sieve limits beyond ~2×10⁹ where π(limit) > 10⁸ primes.
 *   Fix: g_prime_count and all related local counters promoted to int64_t.
 *   All printf format specifiers updated accordingly (%d → %ld).
 *   All implicit int casts in comparisons against g_prime_count removed.
 *   This correction is generic: the practical upper bound is now RAM only.
 *   On a machine with 23 GB available:
 *     bound = 10⁸  →  ~1.0 GB  (sieve 2.1×10⁹)
 *     bound = 5×10⁸ →  ~5.0 GB  (sieve 1.1×10¹⁰)
 *     bound = 10⁹  → ~10.0 GB  (sieve 2.2×10¹⁰)  ← practical maximum
 *   DEFAULT_BOUND and DEFAULT_SIEVE_LIMIT unchanged; pass explicit args
 *   for extended runs.
 *
...
static int64_t   g_prime_count = 0;   /* int64_t: supports π(limit) > 2^31 */
...
static void build_sieve(long limit)
{
    ...
    fprintf(stderr, "  Prime table: %ld primes, %.1f MB allocated\n",
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "Done: %ld primes (largest: %ld), %.1f s\n",
