/*
 * sun41_v12h.c  --  Targeted search for unresolved cases of Sun's Conjecture 4.1(i)
 *
 ...
 *
 * CHANGES v12h (step 3: n = 13e9 .. 16e9):
 *   - PHASE6_BOUND updated to 13000000000 (step-2 bound).
 *   - UNRESOLVED_M: 13 cases remaining after step 2.
 *     Resolved in step 2: 51816, 55468.
 *   - Binary renamed sun41_v12h.
 *
 * CHANGES v12g (step 2: n = 10e9 .. 13e9):
 *   - PHASE6_BOUND = 10000000000 (step-1 bound).
 *   - UNRESOLVED_M: 15 cases remaining after step 1.
 *     Resolved in step 1: 55874, 67203, 82096, 67447, 97251, 98258.
 *   - Binary renamed sun41_v12g.
 *
 * CHANGES v12e (fase 5: n = 6e9 .. 7e9):
 *   - PHASE6_BOUND updated to 6000000000 (fase-4 bound).
 *   - UNRESOLVED_M reduced from 30 to 25 cases (5 resolved in fase 4).
 *     Resolved in fase 4: 19234, 81986, 94429, 97351, 99105.
 *   - Binary renamed sun41_v12e.
 *
 * CHANGES v12d (fase 4: n = 5e9 .. 6e9):
 *   - PHASE6_BOUND updated to 5000000000 (fase-3 bound).
 *   - UNRESOLVED_M reduced from 34 to 30 cases (4 resolved in fase 3).
 *     Resolved in fase 3: 33566, 36303, 72568, 90005.
 *   - Binary renamed sun41_v12d.
 *
 ...
 *       sun41_v12h.c -o sun41_v12h -lm
 *
 * Usage:
 *   ./sun41_v12h [BOUND [BLOCK_SIZE]]
...
/* Step-2 search bound (n-index). v12h continues from here. */
#define PHASE6_BOUND  13000000000LL

/* ================================================================== */
/*  The 13 unresolved m-values after Step 2 (n-bound 13e9)           */
/*  Removed (resolved in step 2): 51816, 55468.                      */
/* ================================================================== */
static const int UNRESOLVED_M[] = {
    22802, 23995, 37249, 37253, 37949, 43297, 48834,
    53963, 66257, 69546, 76868, 82300, 98379
};
...
int main(int argc, char *argv[])
{
    ...
    time_t t0 = time(NULL);
    fprintf(stderr,
        "sun41_v12h -- targeted segmented-sieve search (A247975)\n");
    fprintf(stderr, "  Unresolved cases : %d (after step 2, n-bound 13e9)\n",
            N_UNRESOLVED);
    ...
    /* ---- CSV output ---- */
    printf("# sun41_v12h: targeted search for A247975 unresolved cases\n");
